document.addEventListener("DOMContentLoaded", () => {
    ...
    const phonePhotos = (window.photoData.phonePhotos || []).map((name) => ({
        src: `images/gallery/${toWebJpg(name)}`,
        originalSrc: `images/gallery/${toWebJpg(name)}`,
        label: "iPhone",
        type: "phone"
    }));
    const cameraPhotos = (window.photoData.cameraPhotos || []).map((name) => ({
        src: `images/gallery/${name}`,
        originalSrc: `images/gallery/${name}`,
    ...
    const renderPhotos = (photosToRender) => {
        ...
        gallery.querySelectorAll("img[data-original-src]").forEach((img) => {
            img.addEventListener("error", () => {
                ...
                if (fallback && (img.src.indexOf("/gallery/") !== -1 || img.src.indexOf("/web/") !== -1)) {
                    img.src = fallback;
                }
            }, { once: true });
        });
